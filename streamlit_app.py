...
if st.session_state['HSV_value_text'] is None:
	...
	text = 'Colour name: <span style=\'background-color: #9685ff; padding: 2.5px;\'><strong>   ' + st.session_state['colour_name_text'] + '</strong></span><br>' +\
	...
	for index, row in st.session_state['suggestions_text'].iterrows():
		...
		text += f" with <span style='background-color: #9685ff; padding: 2.5px;'><strong>{other_color}</strong></span>: {category}<br>"
